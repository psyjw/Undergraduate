...
ppCommand n (CmdIf {ciCond = e, ciThen = c1, ciElse = eb, cmdSrcPos = sp}) =
...
ppCommand n (CmdLet {clDecls = ds, clBody = c, cmdSrcPos = sp}) =
    ...
    . ppCommand (n+1) c
ppCommand n (CmdRepeat {crCmd = c, crCond = e, cmdSrcPos = sp}) =
    indent n . showString "CmdRepeat" . spc . ppSrcPos sp . nl
    . ppCommand (n+1) c
    . ppExpression (n+1) e

------------------------------------------------------------------------------
-- Pretty printing of elsebranch
------------------------------------------------------------------------------    

ppElseBranch :: Int -> ElseBranch -> ShowS
ppElseBranch n (EBsingle {ebS = c, ebSrcPos = sp} ) =
    ppCommand (n+1) c
ppElseBranch n (EBelsif {ebCond = e, ebThen = c, ebElse = eb, ebSrcPos = sp} ) =
    ppExpression (n+1) e
    . ppCommand (n+1) c
    . ppElseBranch (n) eb
ppElseBranch n (EBepsilon) =
    indent 0
...
ppExpression n (ExpApp {eaFun = f, eaArgs = es, expSrcPos = sp}) =
    ...
    . ppExpression (n+1) f
    . ppSeq (n+1) ppExpression es
ppExpression n (ExpCond {ecCond = e, ecThen = e1, ecElse = e2, expSrcPos = sp}) =
