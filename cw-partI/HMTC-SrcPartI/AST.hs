...
data ElseBranch
    -- | Conditional command with else ending
    = EBsingle {
          ebS       :: Command,         -- ^ Else-branch
          ebSrcPos  :: SrcPos
    }
    -- | Conditional command with elsif extension 
    | EBelsif {
          ebCond    :: Expression,      -- ^ Condition
          ebThen    :: Command,         -- ^ Then-branch
          ebElse    :: ElseBranch,      -- ^ Else-branch
          ebSrcPos  :: SrcPos
    }
    -- | Conditional command without else ending
    | EBepsilon
    
instance HasSrcPos ElseBranch where
    srcPos = ebSrcPos
...
data Expression
    ...
    -- | Function "a ? b : c" represents conditional command but in an expression manner
    | ExpCond {
          ecCond    :: Expression,      -- ^ Condition
          ecThen    :: Expression,      -- ^ Then-branch
          ecElse    :: Expression,      -- ^ Else-branch
          expSrcPos :: SrcPos
      }
    -- | Literal character
    | ExpLitChar {
