...
scanner cont = P $ scan
    where
        ...
        scanLitChar l c (x : '\'' : s) = if ((fromEnum x >= 32) && (fromEnum x <= 126) && (x /= '\'') && (x /= '\\'))
                                                then retTkn (LitChar x) l c (c + 3) s
                                                else do
                                                        emitErrD (SrcPos l c)
                                                                 ("Lexical error: Illegal \
                                                                 \character "
                                                                 ...
                                                                 ++ " (discarded)")
                                                        scan l (c + 3) s
        scanLitChar l c ('\\' : x : '\'' : s) = if x == 'n' || x == 'r' || x == 't' || x == '\\' || x == '\''
                                                        then retTkn (LitChar (remakeEscape x)) l c (c + 4) s
                                                        else do
                                                                emitErrD (SrcPos l c)
                                                                         ("Lexical error: Illegal \
                                                                         \character "
