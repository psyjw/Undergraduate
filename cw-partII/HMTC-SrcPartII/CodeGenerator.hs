...
execute majl env n (CmdIf {ciCondThens = ecs, ciMbElse = mc2}) = do
    lblFinish <- newName
    mapM (exeOptIf majl env n lblFinish) ecs
    case mc2 of
        Just c2 -> do
                execute majl env n c2
        Nothing -> do
                return ()
    emit (Label lblFinish)
...
exeOptIf :: MSL -> CGEnv -> MTInt -> Name -> (Expression, Command) -> TAMCG ()
exeOptIf majl env n lblFinish (e, c) = do
    lblOptIf <- newName
    evaluate majl env e
    emit (JUMPIFZ lblOptIf)
    execute majl env n c
    emit (JUMP lblFinish)
    emit (Label lblOptIf)
...
evaluate majl env (ExpPrj {epRcd = r, epFld = f, expType = t}) = do
    ...
    emit ADD
evaluate majl env (ExpCond {ecCond = e1, ecTrue = e2, ecFalse = e3, expType = t}) = do
    lblFalse  <- newName
    lblFinish <- newName
