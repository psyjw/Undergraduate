...
chkCmd env (A.CmdIf {A.ciCondThens = ecs, A.ciMbElse = mc2,
                     A.cmdSrcPos=sp}) = do
    ecs' <- mapM (chkOptIf env) ecs                     -- env |- ecs
    mc2' <- case mc2 of
                Just c2 -> do
...
chkOptIf :: Env -> (A.Expression, A.Command) -> D (Expression, Command)
chkOptIf env (e, c) = do
    e' <- chkTpExp env e Boolean                        -- env |- e : Boolean
    c' <- chkCmd env c                                  -- env |- c
    return (e', c')
...
infTpExp env (A.ExpCond {A.ecCond = e1, A.ecTrue = e2, A.ecFalse = e3, A.expSrcPos = sp}) = do
    ...
    (t, e2') <- infNonRefTpExp env e2
    (t, e3') <- infNonRefTpExp env e3
    return (t, ExpCond {ecCond = e1', ecTrue = e2', ecFalse = e3', expType = t, expSrcPos = sp})
